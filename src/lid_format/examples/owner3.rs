...
fn main() {
    ...
    for oi in ords {
        let win = |c: u32| {
            let (s, v) = (g(c, 0x8000), g(c, 0));
            if oi + 1 < s.len() && (s[oi + 1] as usize) <= v.len() {
                v[s[oi] as usize..s[oi + 1] as usize].to_vec()
            } else {
                vec![]
            }
        };
        println!(
            "ord {oi} elem {}: col1 {:?} c01 {:?} c12 {:?}",
    ...
    let c11 = g(0xc11, 0);
    println!("c11[683..695]: {:?}", &c11[683..695.min(c11.len())]);
}
