...
fn main() {
    ...
    for bi in 0..ga.blocks.len() {
        ...
        for col in [
            0x0c08u32, 0x0c10, 0x0c14, 0x0c15, 0x0c16, 0x0c0d, 0x0c0e, 0x0c0f,
        ] {
            let s8 = blk
                .streams
                .iter()
                .find(|x| x.col == col && x.flags == 0x8000)
            ...
            if let Some(s8) = s8 {
                ...
                for d in 1885..=1899u32 {
                    let i = d as usize;
                    if i + 1 < s8.len() {
                        let (st, en) = (s8[i] as usize, s8[i + 1] as usize);
                        if st < en && en <= v0.len() {
                            out.push((d, &v0[st..en]));
                        }
                    }
                ...
                if !out.is_empty() {
                    println!("block {bi} col {col:#x}: {out:?}");
                }
            }
        }
        // global needle scan per col values:
        for s in &blk.streams {
            if s.flags == 0 && needles.iter().any(|n| s.values.contains(n)) {
                println!(
                    "block {bi} col {:x} flags {:x} CONTAINS needle",
                    s.col, s.flags
                );
            }
        }
