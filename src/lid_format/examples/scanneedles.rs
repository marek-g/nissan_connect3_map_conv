...
fn main() {
    ...
    for bi in 0..ga.blocks.len() {
        ...
        for s in &blk.streams {
            for n in &needles {
                let mut cnt = 0usize; let mut first = usize::MAX;
                for (i,&v) in s.values.iter().enumerate() { if v==*n { if first==usize::MAX {first=i} cnt+=1; } }
                if cnt>0 { println!("blk {bi} {}..{} col {:#06x} fl {:#06x} needle {n} x{cnt} first@{first}", blk.elem_start, blk.elem_end, s.col, s.flags); }
            }
        }
    }
}
