...
fn main() {
    ...
    for bi in 134..ga.blocks.len() {
        ...
        let cb1=blk.streams.iter().find(|x|x.col==0xc01&&x.flags==0x4000).map(|s|s.bits.clone()).unwrap();
        ...
        for ord in 0..cb1.len() {
            if cb1[ord] {
                ...
                if has { with += 1 } else { without += 1; missing.push(blk.elem_start + ord as u32); }
            }
        }
    }
