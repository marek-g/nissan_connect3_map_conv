...
fn main() {
    ...
    let s01 = blk
        .streams
        .iter()
        .find(|x| x.col == 0x0c01 && x.flags == 0x8000)
        .map(|s| s.values.clone())
        .unwrap_or_default();
    let v01 = blk
        .streams
        .iter()
        .find(|x| x.col == 0x0c01 && x.flags == 0)
        .map(|s| s.values.clone())
        .unwrap_or_default();
    println!(
        "0xc01: bits {} starts {} vals {}",
        bits.len(),
        s01.len(),
        v01.len()
    );
    ...
    for ord in [1346usize /*67687*/, 1337 /*67678*/] {
        ...
        println!(
            "elem {} pord {p} c01 vals {:?} needles {:?}",
            blk.elem_start + ord as u32,
            &v01[st..en],
            {
                let h: Vec<u32> = v01[st..en]
                    .iter()
                    .cloned()
                    .filter(|v| needles.contains(v))
                    .collect();
                h
            }
        );
