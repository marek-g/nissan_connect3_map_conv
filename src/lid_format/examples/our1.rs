    ...
    for bi in 0..ga.blocks.len() {
        if ga.blocks[bi].elem_start <= sid && sid <= ga.blocks[bi].elem_end {
            ...
            let ob = blk
                ...
                .map(|s| s.bits.clone())
                .unwrap_or_default();
            let po = if !ob.is_empty() {
                b1(&ob, ord)
            } else {
                usize::MAX
            };
            let owners = if po != usize::MAX && po + 1 < s1.len() {
                v1[s1[po] as usize..s1[po + 1] as usize].to_vec()
            } else {
                vec![]
            };
            let s0 = blk
            ...
            let (st, en) = (
                s0[p] as usize,
                s0.get(p + 1).map(|&x| x as usize).unwrap_or(v0.len()),
            );
            println!(
                "sid {sid} owners {owners:?} nums {:?} c11 {:?}",
                &v0[st..en],
                &c11[st..en]
            );
        }
...
fn b1(bits: &[bool], ord: usize) -> usize {
    bits[..=ord].iter().filter(|&&x| x).count() - 1
}
