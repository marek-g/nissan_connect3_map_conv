...
fn main() {
    ...
    for bi in 0..ga.blocks.len() {
        ...
        let bits = blk
            .streams
            .iter()
            .find(|x| x.col == 0x0c11 && x.flags == 0x4000)
            .map(|s| s.bits.clone())
            .unwrap_or_default();
        let vals = blk
            .streams
            .iter()
            .find(|x| x.col == 0x0c11 && x.flags == 0)
    ...
    for bi in 0..ga.blocks.len() {
        ...
        if b12.is_empty() || !b12[ord] {
            println!("elem {elem}: no 0xc12 owner");
            continue;
        }
        ...
        let ow: Vec<u32> = d
            .iter()
            .map(|&x| *owner.get(x as usize).unwrap_or(&0xdeadbeef))
            .collect();
        let hit: Vec<u32> = ow.iter().cloned().filter(|v| needles.contains(v)).collect();
        println!(
            "pord {pord} descrs {:?} owners {:?} {}",
            d,
            ow,
            if hit.is_empty() {
                String::new()
