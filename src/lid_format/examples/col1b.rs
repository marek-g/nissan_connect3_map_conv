...
fn main() {
    ...
    let s1 = blk.streams.iter().find(|x| x.col==1&&x.flags==0x8000).map(|s|s.values.clone()).unwrap();
    let v1 = blk.streams.iter().find(|x| x.col==1&&x.flags==0).map(|s|s.values.clone()).unwrap();
    let c11 = blk.streams.iter().find(|x| x.col==0xc11&&x.flags==0).map(|s|s.values.clone()).unwrap_or_default();
    ...
    for n in &needles {
        for oi in 0..s1.len().saturating_sub(1) {
            ...
            if st<en && en<=v1.len() && v1[st..en].contains(n) {
                println!("needle {n}: owner ord {oi} elem {} window {:?} c11@ord {} {:?}", blk.elem_start+oi as u32, &v1[st..en], oi, c11.get(oi));
            }
        }
    }
}
