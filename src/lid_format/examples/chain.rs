...
fn main() {
    ...
    for bi in 0..ga.blocks.len() {
        ...
        println!("block {bi} range {}..{} ord {ord}", blk.elem_start, blk.elem_end);
        let bit = |col:u32,fl:u32| blk.streams.iter().find(|x| x.col==col&&x.flags==fl).map(|s|s.bits.clone()).unwrap_or_default();
        let val = |col:u32,fl:u32| blk.streams.iter().find(|x| x.col==col&&x.flags==fl).map(|s|s.values.clone()).unwrap_or_default();
        let b12 = bit(0x0c12,0x4000);
        let pord = b12[..ord+1].iter().filter(|&&x|x).count()-1;
        println!("0xc12 present-owner ord {pord} of {}", b12.iter().filter(|&&x|x).count());
        ...
        println!("0xc14 owners={} vals={} starts={}", cnt14, v14.len(), s14.len());
        // find where descr value appears as index into a starts-list:
        for d in descrs { let d=*d as usize;
            if d < v14.len()/2 { println!("  descr {d} -> v14 pair ({},{})", v14[2*d], v14[2*d+1]); }
        }
        ...
        if !s11.is_empty() {
            let (a1,b1)=(s11[p11] as usize, s11.get(p11+1).map(|&x|x as usize).unwrap_or(val(0x0c11,0).len()));
            let r11 = &val(0x0c11,0)[a1..b1];
            println!("0xc11 record descrs: {r11:?}");
            for d in r11 { let d=*d as usize;
                if d/2 < v14.len()/2 && d%2==0 { println!("  r11 {d} -> pair ({},{})", v14[d], v14[d+1]); }
            }
        }
    }
}
